 ...
 * When the agent calls build_x_upload_file, OpenClaw invokes the tool's execute()
...
export class GatewayBridge {
  ...
  /**
   * Called by the build_x_upload_file tool execute() in index.js after a
  ...
  async sendPrompt(sessionId, message, requestId) {
    // Pre-process: resolve build-x-file:// URIs → <BUILD_X_FILE /> tags
    let resolvedMessage = message;
    if (this.fileResolver && typeof message === 'string' && message.includes('build-x-file://')) {
      try {
    ...
    const sessionKey = `build-x:${sessionId}`;
    ...
    const sent = this.gatewayClient.send({
      ...
      params: {
        ...
        idempotencyKey: `build_x_${sessionId}_${Date.now()}`,
      }
    });
