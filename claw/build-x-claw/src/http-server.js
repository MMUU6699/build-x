...
export class BuildXClawHttpServer {
  ...
  start() {
    ...
    this.server.listen(this.port, this.host, () => {
      this.logger?.info?.(`[build-x-claw] HTTP server listening on ${this.host}:${this.port}`);
    });
    this.server.on('error', (err) => {
      this.logger?.error?.(`[build-x-claw] HTTP server error: ${String(err)}`);
    });
  }
  ...
  async _handleChat(req, res) {
    ...
    // all default to "default" (mapped internally to "build-x:default").
  ...
  async _handleHistory(req, res, url) {
    ...
    // Map to the sessionKey format OpenClaw uses internally: "build-x:{rawSessionId}"
    const sessionId = `build-x:${rawSessionId}`;
    ...
    const messages = merged.map(e => {
      ...
      if (Array.isArray(content)) {
        ...
        for (const block of content) {
          ...
          if (btype === 'text') {
            ...
            const uri = block.uri;
            if (uri) {
              const m = uri.match(/^build-x-file:\/\/(.+)$/);
              attachments.push({
      ...
      // Extract <BUILD_X_FILE .../> tags from text content
      if (typeof content === 'string' && content.includes('<BUILD_X_FILE')) {
        const tagRegex = /<BUILD_X_FILE\b([^>]*)\/>/g;
        let match;
        ...
        content = content.replace(/<BUILD_X_FILE\b[^>]*\/>/g, '').trim();
      }
