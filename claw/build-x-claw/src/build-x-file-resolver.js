...
const BUILD_X_FILE_URI_PREFIX = 'build-x-file://';
const BUILD_X_FILE_URI_REGEX = /build-x-file:\/\/([a-f0-9]{24})/g;
const RESOLVE_TIMEOUT_MS = 30_000;
const MAX_FILENAME_LENGTH = 120;

/**
 * Resolves `build-x-file://{fileId}` URIs found in prompts by:
 *   1. Downloading the file from the Build X backend API to local disk
 *   2. Replacing the URI with a `<BUILD_X_FILE .../>` tag the agent can use
 *
 ...
 */
export class BuildXFileResolver {
  constructor({ buildXApiBaseUrl, buildXApiKey, downloadDir, uploadMetaDir, logger }) {
    this.buildXApiBaseUrl = buildXApiBaseUrl;
    this.buildXApiKey = buildXApiKey;
  ...
  /**
   * Scan text for build-x-file:// URIs and return unique fileIds.
   */
  buildResolutionPlan(text) {
    const fileIds = new Set();
    let match;
    const re = new RegExp(BUILD_X_FILE_URI_REGEX.source, 'g');
  ...
  /**
   * Full pipeline: detect → resolve → replace all build-x-file:// URIs in text.
   * Returns the transformed text with <BUILD_X_FILE .../> tags.
   */
  async resolvePrompt(text) {
    ...
    this.logger?.info?.(`[file-resolver] found ${fileIds.length} build-x-file:// refs to resolve`);
    ...
    for (const [fileId, result] of resolutions) {
      const uriPattern = new RegExp(`build-x-file://${fileId}`, 'g');
      const tag = result.ok
        ? buildBuildXFileRefText(result)
        : buildBuildXFileFailedRefText(result);
  ...
  /**
   * Fetch file metadata from the Build X backend.
  ...
  async _fetchFileMeta(fileId) {
    ...
    try {
      const url = `${this.buildXApiBaseUrl}/api/v1/claw/resolve/${encodeURIComponent(fileId)}`;
      const resp = await fetch(url, {
        method: 'GET',
        headers: {
          'X-Claw-Api-Key': this.buildXApiKey || '',
  ...
  async _downloadFile(fileId, meta) {
    ...
    try {
      const downloadUrl = `${this.buildXApiBaseUrl}/api/v1/claw/resolve/${encodeURIComponent(fileId)}/download`;
      const resp = await fetch(downloadUrl, {
        method: 'GET',
        headers: { 'X-Claw-Api-Key': this.buildXApiKey || '' },
        signal: controller.signal,
      });
...
function buildBuildXFileRefText({ fileId, localPath, name }) {
  return `<BUILD_X_FILE type="file" path="${escapeXmlAttribute(localPath)}" name="${escapeXmlAttribute(name)}" id="${escapeXmlAttribute(fileId)}" />`;
}

function buildBuildXFileFailedRefText({ fileId, name, reason }) {
  return `<BUILD_X_FILE type="file" path="" name="${escapeXmlAttribute(name)}" id="${escapeXmlAttribute(fileId)}" status="download_failed" reason="${escapeXmlAttribute(reason)}" />`;
}
