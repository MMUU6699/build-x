...
import { BuildXClawHttpServer } from './http-server.js';
import { BuildXFileResolver } from './build-x-file-resolver.js';

// ─── build_x_upload_file tool definition ───────────────────────────────────────

const BUILD_X_UPLOAD_TOOL_NAME = 'build_x_upload_file';

const BUILD_X_UPLOAD_TOOL_DESCRIPTION =
  'Upload local files from the workspace to the Build X platform so the user can download them. ' +
...
const BUILD_X_UPLOAD_TOOL_SCHEMA = {
  type: 'object',
  required: ['paths'],
 ...
 */
async function uploadFileToBuildX({ filePath, fileName, buildXApiBaseUrl, buildXApiKey }) {
  ...
  try {
    const uploadUrl = `${buildXApiBaseUrl}/api/v1/claw/upload`;
    const response = await fetch(uploadUrl, {
      method: 'POST',
      headers: { 'X-Claw-Api-Key': buildXApiKey },
      body: formData,
...
function getUploadMetaDir(openclawHome) {
  return path.join(openclawHome || '/home/node/.openclaw', 'plugins', 'build-x-claw', UPLOAD_META_DIR_NAME);
}
...
const plugin = {
  id: 'build-x-claw',
  name: 'build-x-claw',
  description: 'Connector plugin that bridges build-x backend with the local OpenClaw Gateway.',
  ...
  register(context) {
    ...
    // Build X backend connection info (available as env vars in the container)
    const buildXApiBaseUrl = process.env.BUILD_X_API_BASE_URL || 'http://backend:8000';
    const buildXApiKey = process.env.BUILD_X_API_KEY || '';

    // ── Register build_x_upload_file as a native OpenClaw plugin tool ────────────
    ...
    context.registerTool({
      name: BUILD_X_UPLOAD_TOOL_NAME,
      description: BUILD_X_UPLOAD_TOOL_DESCRIPTION,
      parameters: BUILD_X_UPLOAD_TOOL_SCHEMA,

      async execute(toolCallId, args) {
        const logger = context.logger;
        const rawPaths = args?.paths || [];
        logger?.info?.(`[build_x_upload_file] called toolCallId=${toolCallId} paths=${JSON.stringify(rawPaths)}`);
        ...
        for (const { filePath, fileName, size } of validFiles) {
          try {
            const fileInfo = await uploadFileToBuildX({ filePath, fileName, buildXApiBaseUrl, buildXApiKey });
            uploaded.push(fileInfo);
            logger?.info?.(`[build_x_upload_file] uploaded ${fileName} -> file_id=${fileInfo.file_id}`);
            ...
            logger?.warn?.(`[build_x_upload_file] upload failed for ${fileName}: ${msg}`);
          }
        }
        ...
        // 6) Return resource_link content blocks with build-x-file:// URIs
        const content = uploaded.map(u => ({
          type: 'resource_link',
          uri: `build-x-file://${u.file_id}`,
          name: u.filename,
        ...
        logger?.info?.(`[build_x_upload_file] result=${JSON.stringify(result)}`);
    ...
    context.registerService({
      ...
      start: (runtime) => {
        ...
        if (cfg.log.enabled) {
          logger?.info?.(`[build-x-claw] starting with gateway=${cfg.gateway.url} server=${cfg.server.host}:${cfg.server.port}`);
        }
        ...
        // File resolver: build-x-file:// → download → <BUILD_X_FILE />
        const fileDownloadDir = path.join(workspaceDir, 'download');
        const fileResolver = new BuildXFileResolver({
          buildXApiBaseUrl,
          buildXApiKey,
        ...
        httpServer = new BuildXClawHttpServer({
