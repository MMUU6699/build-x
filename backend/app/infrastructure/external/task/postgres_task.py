"""In-process task implementation backed by Postgres for I/O.

Mirrors the former ``RedisStreamTask``: the task runs as an asyncio task in
the current process; only the input/output streams live in Postgres
(``task_streams``). The task registry is process-local.
"""
import asyncio
import uuid
import logging
...
class PostgresStreamTask(Task):
    """In-process task implementation backed by Postgres streams for I/O."""

    _task_registry: Dict[str, 'PostgresStreamTask'] = {}
    _runner_factory: Optional[TaskRunnerFactory] = None

    def __init__(self, params: Dict[str, Any]):
        """Initialize with serializable runner parameters.
        ...
        self._input_stream = PostgresStreamQueue(input_stream_name)
        self._output_stream = PostgresStreamQueue(output_stream_name)

        PostgresStreamTask._task_registry[self._id] = self
    ...
    async def run(self) -> None:
        ...
        if self._done:
            if self._runner is None:
                if PostgresStreamTask._runner_factory is None:
                    raise RuntimeError("No TaskRunnerFactory registered for PostgresStreamTask")
                self._runner = await PostgresStreamTask._runner_factory.create_runner(self._params)
    ...
    def _cleanup_registry(self) -> None:
        ...
        if self._id in PostgresStreamTask._task_registry:
            del PostgresStreamTask._task_registry[self._id]
    ...
    @classmethod
    async def get(cls, task_id: str) -> Optional['PostgresStreamTask']:
        """Get a task by its ID from the local registry."""
        return cls._task_registry.get(task_id)

    @classmethod
    def create(cls, params: Dict[str, Any]) -> "PostgresStreamTask":
    ...
    def __repr__(self) -> str:
        ...
        return f"PostgresStreamTask(id={self._id}, done={self._done})"
