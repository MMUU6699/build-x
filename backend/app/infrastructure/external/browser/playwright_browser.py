...
class PlaywrightBrowser:
    ...
    async def _extract_interactive_elements(self) -> List[str]:
        ...
        interactive_elements = await self.page.evaluate("""() => {
            ...
            for (let i = 0; i < elements.length; i++) {
                ...
                // Only add data-build-x-id attribute to elements that meet the conditions
                element.setAttribute('data-build-x-id', `build-x-element-${validElementIndex}`);
                                                        
                // Build selector - using only data-build-x-id
                const selector = `[data-build-x-id="build-x-element-${validElementIndex}"]`;
    ...
    async def _get_element_by_index(self, index: int) -> Optional[Any]:
        """Get element by index using data-build-x-id selector
        ...
        # Use data-build-x-id selector
        selector = f'[data-build-x-id="build-x-element-{index}"]'
