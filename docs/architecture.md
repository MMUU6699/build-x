...
Claw 是 Build X 深度集成的 [OpenClaw](https://github.com/anthropics/openclaw) AI 助手模块，以 **Build X × Claw** 的形式为用户提供独立的聊天体验。

**架构概览：**

- **claw/ 容器镜像：**基于 `ghcr.io/openclaw/openclaw:latest` 构建，内置 `build-x-claw` Node 插件，运行 OpenClaw Gateway 并支持 TTL 自动过期。
...
- **Frontend 集成：**当 `claw_enabled` 配置开启时，左侧边栏出现 "Build X Claw" 入口，路由至 `/chat/claw` 页面，通过 WebSocket 实现实时聊天。
- **build-x-claw 插件：**桥接 OpenClaw Gateway 与 Build X 后端，提供 HTTP 服务、`build_x_upload_file` 工具、文件解析与会话历史读取等能力。
