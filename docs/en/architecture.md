...
Claw is Build X's deeply integrated [OpenClaw](https://github.com/anthropics/openclaw) AI assistant module, delivering the **Build X × Claw** experience as a standalone chat interface.

**Architecture Overview:**

- **claw/ container image:** Built on `ghcr.io/openclaw/openclaw:latest`, includes the `build-x-claw` Node plugin, runs the OpenClaw Gateway with TTL auto-expiry support.
...
- **Frontend integration:** When `claw_enabled` is turned on, a "Build X Claw" entry appears in the sidebar, routing to the `/chat/claw` page with real-time chat over WebSocket.
- **build-x-claw plugin:** Bridges the OpenClaw Gateway with the Build X backend, providing an HTTP server, the `build_x_upload_file` tool, file resolution, and session history reads.
