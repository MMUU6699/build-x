...
> Canonical guide for AI coding agents working on the **Build X × Claw** codebase.

---

## Project Overview

Build X × Claw is a general-purpose AI Agent system with an integrated [OpenClaw](https://github.com/anthropics/openclaw) AI assistant, comprising five services:
...
├── claw/              # Claw service (OpenClaw Gateway + build-x-claw plugin)
│   └── build-x-claw/   # Node.js plugin bridging OpenClaw with Build X backend
...
- MongoDB data persists in volume `build-x-mongodb-data`. Wipe with `./dev.sh down -v`.
